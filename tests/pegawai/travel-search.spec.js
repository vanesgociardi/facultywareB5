...
test.describe('Pegawai Travel Search and Filter', () => {
  ...
  test.afterEach(async ({ page }) => {
    // Cleanup if a travel request was created
    if (createdTravelId) {
      page.on('dialog', async (dialog) => {
        await dialog.accept();
      });
      await page.goto(`/pegawai/travels/delete/${createdTravelId}`);
  ...
  test('should search by destination and purpose', async ({ page }) => {
    ...
    // Verify the table does not contain our search target and shows empty state or doesn't show the travel
